...
func DiscoverCSVSchema(provider Provider, model string, header string, sampleRows []string) (models.CSVSchema, int, error) {
  ...
  Return ONLY a valid JSON object matching these exact keys.`
	...
	if err != nil {
		return models.CSVSchema{}, 0, fmt.Errorf("failed to discover CSV schema: %w", err)
	}
	...
	if err := json.Unmarshal([]byte(cleanJSON), &schema); err != nil {
		return models.CSVSchema{}, resp.TotalTokens, fmt.Errorf("failed to parse CSV schema from LLM: %w (raw response: %s)", err, resp.Content)
	}

	return schema, resp.TotalTokens, nil
}
