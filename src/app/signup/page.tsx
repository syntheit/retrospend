...
type InviteState = "no-code" | "validating" | "success";
...
function SignupForm() {
	...
	const [inviteState, setInviteState] = useState<InviteState>("no-code");
	...
	const handleValidateInviteCode = useCallback(
		async (code: string) => {
			...
			try {
				...
				if (result.valid) {
					...
					// Set cookie
					// biome-ignore lint/suspicious/noDocumentCookie: Controlled cookie setting for invite functionality
					document.cookie = `retro_invite_code=${code}; path=/; max-age=86400`; // 24 hours
				} else {
					setInviteState("no-code");
	...
	const handleInviteCodeChange = (value: string) => {
		...
		setInviteError("");
		if (value.length === 8) {
			handleValidateInviteCode(value);
		} else if (inviteState !== "no-code") {
			setInviteState("no-code");
		}
	};
	useEffect(() => {
		const codeParam = searchParams.get("code");
		if (codeParam && inviteState === "no-code") {
			setInviteCode(codeParam.toUpperCase());
			handleValidateInviteCode(codeParam.toUpperCase());
		}
	}, [searchParams, inviteState, handleValidateInviteCode]);
			...
			<Card className="w-full max-w-md">
				...
				<CardContent>
					<form className="space-y-4" onSubmit={handleSubmit}>
						{/* Invite Code Section - only show if invite-only mode is enabled */}
						{inviteOnlyEnabled && (
