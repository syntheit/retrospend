...
export function RecurringCalendar({
	...
	const next30Days = React.useMemo(
		() => Array.from({ length: 30 }, (_, i) => addDays(now, i)),
			...
			next30Days
				.flatMap((date: Date) => {
					return (templates ?? [])
						.filter(
							(t: RecurringTemplate) =>
								t.isActive && isProjectedOnDate(t.frequency, t.nextDueDate, date),
						)
						.map((t: RecurringTemplate) => ({ ...t, date }));
				})
				.sort((a, b) => a.date.getTime() - b.date.getTime()),
		[next30Days, templates],
	);
		...
		() =>
			selectedDate
				? (templates ?? [])
						.filter(
							(t: RecurringTemplate) =>
								t.isActive && isProjectedOnDate(t.frequency, t.nextDueDate, selectedDate),
						)
	...
	const paymentDates = React.useMemo(() => {
		...
		// Project each active template across the next 60 days for calendar dots
		for (let i = 0; i < 60; i++) {
			const date = addDays(now, i);
			for (const t of active) {
				if (isProjectedOnDate(t.frequency, t.nextDueDate, date)) {
					dates.push(date);
					break; // one dot per day is enough
				}
			}
		}
		return dates;
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [templates, now.toDateString()]);
			...
			<div className="flex justify-center p-4 pb-2">
				<Calendar
					...
					classNames={{
						...
						button_previous:
							"h-7 w-7 cursor-pointer transition-colors bg-transparent text-muted-foreground hover:text-foreground pointer-events-auto",
						button_next:
							"h-7 w-7 cursor-pointer transition-colors bg-transparent text-muted-foreground hover:text-foreground pointer-events-auto",
					}}
					mode="single"
