...
type DemoActivityItem = RouterOutputs["dashboard"]["getRecentActivity"][number];
...
const _DAILY_VAR_SPEND = [
	68, 94, 41, 112, 83, 55, 120, 38, 76, 145, 62, 98, 44, 87, 110,
];
...
export const DEMO_CATEGORY_BREAKDOWN: CategorySegment[] =
	DEMO_CATEGORIES_RAW.map((c) => ({
		key: c.key,
		name: c.name,
		value: c.value,
		color: c.color,
...
export const DEMO_RECENT_ACTIVITY: DemoActivityItem[] = [
	...
	},
	{
		type: "shared",
		id: "demo-shared-2",
		title: "Airbnb Brooklyn",
		amount: 210.0,
		currency: "USD",
		amountInUSD: 210.0,
		date: new Date(2026, 2, 11),
		category: { id: "5", name: "Travel", color: "sky", icon: null },
		sharedContext: {
			totalAmount: 420.0,
			participantCount: 2,
			paidByName: "You",
...
export function generateDemoWealthHistory() {
	...
	for (let i = 365; i >= 0; i--) {
		...
		let currentLiabilities =
			startLiabilities + (endLiabilities - startLiabilities) * progress;

		// Add some market cycle simulation (sine waves) plus daily volatility for assets
		const marketCycle =
			Math.sin(progress * Math.PI * 5) * 8000 +
			Math.cos(progress * Math.PI * 12) * 3000;
		...
		currentAssets +=
			(marketCycle + dailyVolatility + bigSwing) * noiseMultiplier;
