...
async function main() {
	...
	const counts = userCategoryCounts
		.map((uc) => uc._count.userId)
		.sort((a, b) => a - b);
	const uniqueCounts = [...new Set(counts)];

	uniqueCounts.forEach((count) => {
		const usersWithCount = userCategoryCounts.filter(
			(uc) => uc._count.userId === count,
		).length;
	...
	const expectedDefaults = (await import("../src/lib/constants"))
		.DEFAULT_CATEGORIES.length;

	console.log(`\nExpected minimum categories per user: ${expectedDefaults}`);
	console.log(`Total users: ${totalUsers}`);

	const usersWithMinCategories = userCategoryCounts.filter(
		(uc) => uc._count.userId >= expectedDefaults,
	).length;
	console.log(
		`Users with at least ${expectedDefaults} categories: ${usersWithMinCategories}/${totalUsers}`,
	);
}
