...
async function main() {
	...
	console.log(
		`ℹ️ Mode: ${DRY_RUN ? "DRY RUN (No changes)" : "LIVE (Will delete data)"}`,
	);
	console.log("----------------------------------------");
	...
	for (const snap of snapshots) {
		...
		if (Math.abs(usd) > Math.abs(balance) * 1.5) {
			...
			console.log(
				`❌ Found Bad Snapshot [${dateStr}]: Asset ${snap.accountId.slice(0, 8)}... (${currency})`,
			);
			console.log(
				`   Bal: ${balance.toLocaleString()} ${currency} -> USD: $${usd.toLocaleString()}`,
			);
	...
	if (DRY_RUN) {
		...
		console.log(
			"To delete these records, edit this script to set DRY_RUN = false.",
		);
		return;
	}
	...
	console.log(
		"The system will regenerate correct history on the next dashboard load/update.",
	);
}
